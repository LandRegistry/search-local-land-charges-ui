...
class TestCheckMaintenance(TestCase):
    ...
    def test_no_maintenance(self, mock_local_authority_api_service):
        with app.app_context():
            with app.test_request_context():
                g.trace_id = "test"
                mock_local_authority_api_service.get_organisations.return_value = [{"maintenance": False}]
    ...
    def test_maintenance(self, mock_local_authority_api_service):
        with app.app_context():
            with app.test_request_context():
                g.trace_id = "test"
                mock_local_authority_api_service.get_organisations.return_value = [{"maintenance": True}]
