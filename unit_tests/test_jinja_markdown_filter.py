...
class TestJinjaMarkdownFilter(unittest.TestCase):
    ...
    def check_rendering(self, markdown_to_html):
        for markdown in markdown_to_html:
            with app.test_request_context("/"):
                self.assertEqual(
                    render_template_string("{{contents|markdown}}", contents=markdown).strip(),
                    markdown_to_html.get(markdown),
                )

    @mock.patch("server.custom_extensions.jinja_markdown_filter.main.JinjaMarkdownFilter.init_app")
    ...
    def test_render_returns_govuk_html_for_paragraph(self):
        html = '<p class="govuk-body">Foo</p><p class="govuk-body">Bar</p><p class="govuk-body">Wibble</p>'
        markdown_to_html = {
            "Foo\n\nBar\n\nWibble": html,
        }
    ...
    def test_render_returns_govuk_html_for_link(self):
        link = '<p class="govuk-body"><a class="govuk-link" href="http://foo.com">Foo</a></p>'
        link_title = '<p class="govuk-body"><a class="govuk-link" href="http://foo.com" title="Title here">Foo</a></p>'
        markdown_to_html = {
            "[Foo](http://foo.com)": link,
            '[Foo](http://foo.com "Title here")': link_title,
        }
