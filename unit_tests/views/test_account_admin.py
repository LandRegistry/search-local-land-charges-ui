...
class TestAccountAdmin(TestCase):
    ...
    def test_register_post_409(self, mock_account, mock_register):
        ...
        mock_account.return_value.register.return_value = {"status": 409}
        ...
        self.assertEqual(
            mock_register.return_value.email_addresses.errors,
    ...
    def test_activate_account_expired(self, mock_account):
        ...
        self.assertEqual(
            result.location,
            url_for("account_admin.expired_activation", page_type="expired-activation"),
        )
        mock_account.return_value.validate_token.assert_called_with("antoken")
    ...
    def test_activate_account_error(self, mock_account):
        with self.client.session_transaction() as sess:
            sess["registered_email"] = "anemail@email.com"
        mock_account.return_value.validate_token.return_value = {"status": "Rudyard Kipling"}
        result = self.client.get(url_for("account_admin.activate_account", activate_token="antoken"))
        self.assertEqual(result.status_code, 302)
        self.assertEqual(
            result.location,
            url_for("account_admin.expired_activation", page_type="invalid-activation"),
        )
        mock_account.return_value.validate_token.assert_called_with("antoken")
        mock_account.return_value.activate_user.assert_not_called()
    ...
    def test_change_password_with_token_get_invalid(self, mock_account):
        ...
        self.assertEqual(
            result.location,
            url_for("account_admin.reset_password", page_type="invalid-password-link"),
        )
    ...
    def test_expired_activation_with_token_get_expired(self, mock_account):
        ...
        self.assertEqual(
            result.location,
            url_for("account_admin.reset_password", page_type="expired-password-link"),
        )
        mock_account.return_value.validate_token.assert_called_with("anpwtoken")
    ...
    def test_expired_activation_with_token_post_blacklisted(self, mock_account, mock_form):
        ...
        self.assertEqual(
            mock_form.return_value.passwords.errors,
            {"passwords": ["This password is not secure enough"]},
        )
    ...
    def test_reset_password_get_expired(self, mock_form):
        ...
        result = self.client.get(url_for("account_admin.reset_password", page_type="expired-password-link"))
        self.assertEqual(result.status_code, 200)
