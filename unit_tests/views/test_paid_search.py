...
from server.views.paid_search import (
    continue_to_payment,
    search_description_continue,
    search_description_find,
)
...
class TestPaidSearch(TestCase):
    ...
    def test_search_area_description_with_address(self, mock_validate):
        ...
        self.assertEqual(
            result.location,
            url_for("paid_search.confirm_address", enc_search_id="anencodedid"),
        )
        mock_validate.assert_called_with("anencodedid")
    ...
    def test_search_area_description_post(self, mock_find, mock_form, mock_validate):
        ...
        result = self.client.post(
            url_for(
                "paid_search.search_area_description",
    ...
    def test_search_description_find_error(self, mock_search):
        mock_search.return_value.process.return_value = {
            "status": "error",
            "search_message": "anerror",
        }
        mock_form = MagicMock()
    ...
    def test_search_description_continue_no_desc(self, mock_continue):
        ...
        result = search_description_continue(mock_form, mock_state, "anencodedid")
        self.assertIsNone(result)
        self.assertEqual(mock_form.search_area_description.errors, ["Describe the search location"])
        mock_continue.assert_not_called()
    ...
    def test_continue_to_payment(self, mock_state, mock_pay):
        with super_test_context(app):
            ...
            mock_pay.return_value.request_payment.assert_called_with(
                ...
                ANY,
                "Official search result of local land charges",
                "anencsearchid",
            )
            session["paid_searches"]["anencsearchid"] = {
                "payment_state": "anpaymentstate",
    ...
    def test_process_pay_found(
        ...
        self.assertEqual(
            result.location,
            url_for("paid_search.get_paid_search", enc_search_id="anencodedid"),
        )
        mock_clear.assert_called()
        mock_gov_pay.return_value.get_payment.assert_called_with("anid")
        mock_validate.assert_called_with("anencodedid")
        ...
        with self.client.session_transaction() as sess:
            self.assertEqual(
                sess["paid_searches"]["anencodedid"]["search_state"].charges,
                ["some", "charges"],
            )
    ...
    def test_get_paid_search_payment_success(self, mock_paid_utils):
        ...
        mock_paid_utils.request_search_generation.assert_called_with(
            {"an": "extent"}, "anaddress", None, contact_id="mock_user"
        )
        mock_paid_utils.pre_associate_search.assert_called_with(
            "ansearchref",
            None,
            {"an": "extent"},
            "anaddress",
        ...
        with self.client.session_transaction() as sess:
            self.assertEqual(
                sess["paid_searches"]["anencodedid"]["search_state"].search_reference,
                "ansearchref",
            )
    ...
    def test_get_paid_search_payment_paid_fail(self, mock_paid_utils):
        ...
        mock_paid_utils.get_search_result.assert_called_with(
            "ansearchref",
            None,
            {"an": "extent"},
            "anaddress",
    ...
    def test_get_paid_search_result(self, mock_storage, mock_paid_utils, mock_audit):
        ...
        mock_paid_search.charges = [{"documents-filed": "somedocuments"}]
        ...
        with self.client.session_transaction() as sess:
            ...
            self.assertEqual(
                sess["paid_searches"]["anencodedid"]["paid_search_item"],
                mock_paid_search,
            )
    ...
    def test_download_charges_json(self, mock_converter, mock_send_file):
        ...
        with self.client.session_transaction() as sess:
            sess["paid_searches"] = {"anencodedid": {"paid_search_item": mock_paid_search}}
        result = self.client.get(
            url_for(
                "paid_search.download_charges",
                enc_search_id="anencodedid",
    ...
    def test_supporting_document_list_no_paid(self):
        with self.client.session_transaction() as sess:
            sess["paid_searches"] = {"anencodedid": {}}
    ...
    def test_supporting_document_list_ok(self, mock_paginate):
        mock_paid_search = PaidSearchItem()
        mock_paid_search.charges = [{"local-land-charge": 123, "documents-filed": {"some": "documents"}}]
        with self.client.session_transaction() as sess:
            sess["paid_searches"] = {"anencodedid": {"paid_search_item": mock_paid_search}}
    ...
    def test_charge_supporting_documents_no_search(self):
        ...
        result = self.client.get(
            url_for(
                "paid_search.charge_supporting_documents",
                enc_search_id="anencodedid",
    ...
    def test_charge_supporting_documents_no_docs(self):
        mock_paid_search = PaidSearchItem()
        mock_paid_search.charges = [{"local-land-charge": 123}]
        with self.client.session_transaction() as sess:
            sess["paid_searches"] = {"anencodedid": {"paid_search_item": mock_paid_search}}
        result = self.client.get(
            url_for(
    ...
    def test_charge_supporting_documents_docs(self, mock_storage):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertIn("Downloading light obstruction notice documents for LLC-3Z.", result.text)
        mock_storage.return_value.get_external_url.assert_called_with("ansubdirectory", "anbucket")

    @patch("server.views.paid_search.StorageAPIService")
    @patch("server.views.paid_search.request")
    def test_charge_supporting_documents_docs_ajax(self, mock_request, mock_storage):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertEqual("andocumenturl", result.text)
        mock_storage.return_value.get_external_url.assert_called_with("ansubdirectory", "anbucket")
