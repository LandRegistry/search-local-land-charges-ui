...
from server.views.search import (
    get_charge_items,
    property_address_selected,
    search_by_area,
    street_address_selected,
)
...
class TestSearch(TestCase):
    ...
    def test_search_by_post_code_address_post_no_results(
        self, mock_sllc, mock_form, mock_start, mock_by_text, mock_flash
    ):
        mock_form.return_value.validate_on_submit.return_value = True
        mock_form.return_value.search_term.data = "ansearchterm"
    ...
    def test_property_address_selected_mig_fail(self, mock_check, mock_by_uprn, mock_merge):
        mock_by_uprn.return_value.process.return_value = {"data": [{"index_map": {"features": ["some", "features"]}}]}
        ...
        self.assertEqual(
            ...
            "Failed to check authority intersection migration status",
        )
    ...
    def test_property_address_selected_not_migrated(self, mock_redirect, mock_check, mock_by_uprn, mock_merge):
        with super_test_context(app):
            mock_by_uprn.return_value.process.return_value = {
                "data": [{"index_map": {"features": ["some", "features"]}}]
            }
            mock_merge.return_value = {"some": "geojson"}
            mock_check.process.return_value = {
                "flag": "something",
                "includes_migrated": True,
            }
    ...
    def test_search_by_inspire_id_post_inspire_500(self, mock_new_search, mock_inspire, mock_form):
        mock_form.return_value.validate_on_submit.return_value = True
        mock_form.return_value.inspire_id.data = "aninspireid"
        mock_inspire.return_value.process.return_value = {"status": 500}
        result = self.client.post(url_for("search.search_by_inspire_id"))
        self.assertEqual(result.status_code, 500)
        self.assertIn("Sorry, there is a problem with the service", result.text)
        mock_inspire.return_value.process.assert_called_with("aninspireid")
    ...
    def test_inspire_search_results_good(self, mock_llc):
        with self.client.session_transaction() as sess:
            sess["search_state"] = SearchState()
            sess["search_state"].charges = [{"geometry": {"some": "geometry"}, "item": "anchargeitem"}]
    ...
    def test_confirm_search_area_check_warning_get(self, mock_check):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertIn(
            "This search area is near a local authority area that is not yet available on this service",
            result.text,
        )
        mock_check.process.assert_called_with({"some": "extent"})
    ...
    @patch("server.views.search.SearchLocalLandChargeService")
    def test_search_by_area_200_with_charges(
        self,
        mock_sllc,
        mock_report,
        mock_local_auth,
        ...
        with super_test_context(app):
            ...
            mock_redirect.return_value = "anredirect"
            result = search_by_area({"features": [{"geometry": {"an": "geometry"}}]}, "anaddress", True)
            mock_by_area.return_value.process.assert_called_with(
                {"features": [{"geometry": {"an": "geometry"}}]},
                results_filter="cancelled",
            )
            mock_get_items.assert_called_with([{"an": "charge"}])
            mock_local_auth.get_authorities_by_extent.assert_called_with(
                {"type": "GeometryCollection", "geometries": [{"an": "geometry"}]}
            )
            ...
            self.assertEqual(
                session["search_state"].search_extent,
                {"features": [{"geometry": {"an": "geometry"}}]},
            )
    ...
    def test_search_by_area_404(
        ...
        with super_test_context(app):
            ...
            mock_by_area.return_value.process.return_value = {"status": 404}
            mock_local_auth.get_authorities_by_extent.return_value = {"anauthority": True}
            mock_sllc.return_value.save_free_search.return_value = {"id": "anid"}
            mock_redirect.return_value = "anredirect"
            result = search_by_area({"features": [{"geometry": {"an": "geometry"}}]}, "anaddress", True)
    ...
    def test_search_by_area_507(self, mock_sllc, mock_by_area, mock_render, mock_check):
        with super_test_context(app):
            ...
            mock_by_area.return_value.process.return_value = {"status": 507}
            mock_sllc.return_value.save_free_search.return_value = {"id": "anid"}
    ...
    def test_search_by_area_bad(self, mock_sllc, mock_by_area, mock_render, mock_check):
        with super_test_context(app):
            ...
            session["search_state"] = None
            session["unmerged_extent"] = {"unmerged": "extent"}
            mock_by_area.return_value.process.return_value = {"status": 666}
            mock_sllc.return_value.save_free_search.return_value = {"id": "anid"}
            mock_render.return_value = "anrender"
            mock_check.under_maintenance.return_value = True
            with self.assertRaises(ApplicationError):
                search_by_area({"features": [{"geometry": {"an": "geometry"}}]}, "anaddress", True)
            mock_by_area.return_value.process.assert_called_with(
                {"features": [{"geometry": {"an": "geometry"}}]},
                results_filter="cancelled",
            )
    ...
    def test_no_information_available_fail_maintenance(self, mock_render):
        with self.client.session_transaction() as sess:
            ...
            sess["authority_data"] = {
                ...
                "plus_buffer": {"maintenance_list": ["some", "authorities"]},
                "migrated_list": [],
            }
        result = self.client.get(url_for("search.no_information_available"))
        self.assertEqual(result.status_code, 200)
    ...
    def test_no_information_available_fail_scotland(self, mock_render):
        with self.client.session_transaction() as sess:
            ...
            sess["authority_data"] = {
                "flag": "fail_scotland",
                "plus_buffer": {"non_migrated_list": ["some", "authorities"]},
                "migrated_list": [],
            }
        result = self.client.get(url_for("search.no_information_available"))
        self.assertEqual(result.status_code, 200)
    ...
    def test_define_search_area_get_ok(self, mock_form, mock_render, mock_check):
        ...
        mock_render.assert_called_with(
            ...
            style="find",
            information='{"anunmerged": "extent"}',
            coordinate_search=None,
            zoom_extent='{"an": "extent"}',
            local_authority_boundary_url=url_for("ajax.local_authority_service_boundingbox", authority="anauthority"),
    ...
    @patch("server.views.search.CheckMigrationStatus")
    @patch("server.views.search.DefineSearchAreaForm")
    def test_define_search_area_post_pass(self, mock_form, mock_migration, mock_by_area, mock_merge):
        mock_form.return_value.validate_on_submit.return_value = True
        ...
        with self.client.session_transaction() as sess:
            ...
            sess["search_state"].address = "anaddress"
            sess["zoom_to_location"] = {"an": "location"}
        mock_migration.process.return_value = {"flag": "pass"}
    ...
    @patch("server.views.search.CheckMigrationStatus")
    @patch("server.views.search.DefineSearchAreaForm")
    def test_define_search_area_post_warning(self, mock_form, mock_migration, mock_by_area, mock_merge):
        mock_form.return_value.validate_on_submit.return_value = True
        mock_form.return_value.saved_features.data = json.dumps({"saved": "features"})
        with self.client.session_transaction() as sess:
            sess["unmerged_extent"] = {"anunmerged": "extent"}
            ...
            sess["search_state"].address = "anaddress"
            sess["zoom_to_location"] = {"an": "location"}
        mock_migration.process.return_value = {"flag": "warning"}
        mock_by_area.return_value = redirect("anurl")
        mock_merge.return_value = {"anmerged": "extent"}
        result = self.client.post(url_for("search.define_search_area", page_style="find"))
        self.assertEqual(result.status_code, 302)
    ...
    @patch("server.views.search.CheckMigrationStatus")
    @patch("server.views.search.DefineSearchAreaForm")
    def test_define_search_area_post_bad(self, mock_form, mock_migration, mock_by_area, mock_merge):
        mock_form.return_value.validate_on_submit.return_value = True
        mock_form.return_value.saved_features.data = json.dumps({"saved": "features"})
        with self.client.session_transaction() as sess:
            sess["unmerged_extent"] = {"anunmerged": "extent"}
            ...
            sess["search_state"].address = "anaddress"
            sess["zoom_to_location"] = {"an": "location"}
        mock_migration.process.return_value = {"flag": "badness"}
        mock_by_area.return_value = redirect("anurl")
        mock_merge.return_value = {"anmerged": "extent"}
        result = self.client.post(url_for("search.define_search_area", page_style="find"))
        self.assertEqual(result.status_code, 302)
