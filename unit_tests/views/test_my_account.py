...
class TestMyAccount(TestCase):
    ...
    @patch("server.app.has_free_searches_to_repeat")
    @patch("server.app.has_paid_searches_to_repeat")
    @patch("server.views.my_account.SearchLocalLandChargeService")
    @patch("server.views.my_account.flash")
    def test_my_account_page(self, mock_flash, mock_sllc, mock_show_paid_repeat, mock_show_free_repeat):
        mock_show_paid_repeat.return_value = False
        mock_show_free_repeat.return_value = False
    ...
    def test_change_password_post_blacklisted(self, mock_account, mock_auth, mock_form):
        ...
        self.assertIn("Error: Change your password", result.text)
        self.assertEqual(
            mock_form.return_value.new_passwords.errors,
    ...
    def test_active_searches_get_no_search(
        self,
        mock_paginate,
        mock_sllc,
        mock_form,
        mock_show_paid_repeat,
        mock_show_free_repeat,
    ):
        mock_show_paid_repeat.return_value = False
        mock_show_free_repeat.return_value = False
        ...
        mock_sllc.return_value.get_paid_search_items.return_value = [
            mock_search_1,
            mock_search_2,
        ]
    ...
    def test_active_searches_get_search(
        self,
        mock_sins,
        mock_paginate,
        mock_sllc,
    ...
    def test_view_search(self, mock_fernet, mock_sllc):
        ...
        self.assertEqual(
            result.location,
            url_for("paid_search.get_paid_search", enc_search_id="anencryptedthing"),
        )
        ...
        with self.client.session_transaction() as sess:
            sess["paid_searches"]["anencryptedthing"] = {
                "paid_search_item": mock_search,
    ...
    @patch("server.views.my_account.SearchLocalLandChargeService")
    @patch("server.views.my_account.calculate_pagination_info")
    def test_expired_searches_get_no_search(
        self,
        mock_paginate,
        mock_sllc,
        mock_form,
    ...
    @patch("server.views.my_account.calculate_pagination_info")
    @patch("server.views.my_account.search_in_searches")
    def test_expired_searches_get_search(
        self,
        mock_sins,
        mock_paginate,
        mock_sllc,
    ...
    def test_repeat_search_refund(self, mock_flash, mock_sllc, mock_gov_pay):
        ...
        mock_sllc.return_value.get_paid_search_item.return_value = mock_search
        mock_gov_pay.return_value.get_payment.return_value = {"refund_summary": {"status": "FULL"}}
        result = self.client.get(url_for("my_account.repeat_search", search_id="ansearchid"))
        self.assertEqual(result.status_code, 302)
        self.assertEqual(
            result.location,
            url_for("my_account.paid_search_review", search_id="ansearchid"),
        )
        mock_sllc.return_value.get_paid_search_item.assert_called_with("mock_user", "ansearchid")
        mock_gov_pay.return_value.get_payment.assert_called_with("anpaymentid")
        mock_flash.assert_called_with(
            "You cannot request a repeat on this search as it has been refunded to you.<br>"
    ...
    def test_repeat_search_maintenance(self, mock_mig_status, mock_sllc, mock_gov_pay):
        ...
        mock_sllc.return_value.get_paid_search_item.return_value = mock_search
        mock_gov_pay.return_value.get_payment.return_value = {"refund_summary": {"status": "NOT"}}
    ...
    def test_repeat_search_ok(
        ...
        self.assertEqual(
            result.location,
            url_for("paid_search.get_paid_search", enc_search_id="encryptedthing"),
        )
        mock_sllc.return_value.get_paid_search_item.assert_called_with("mock_user", "ansearchid")
        mock_gov_pay.return_value.get_payment.assert_called_with("anpaymentid")
        mock_mig_status.process.assert_called_with({"an": "extent"})
    ...
    @patch("server.views.my_account.calculate_pagination_info")
    @patch("server.views.my_account.SearchFreeSearchesForm")
    def test_free_searches_post_nowt(
        self,
        mock_form,
        mock_paginate,
        mock_sllc,
        ...
        self.assertEqual(
            ["You have entered an invalid Search ID. Check it and try again"],
            mock_form.return_value.search_term.errors,
        )
        mock_paginate.assert_called_with(None, "my_account.free_searches", 15, 1, no_of_pages=1, no_of_items=1)
    ...
    @patch("server.views.my_account.calculate_pagination_info")
    @patch("server.views.my_account.SearchFreeSearchesForm")
    def test_free_searches_post_found(
        self,
        mock_form,
        mock_paginate,
        mock_sllc,
    ...
    @patch("server.app.has_paid_searches_to_repeat")
    @patch("server.views.my_account.SearchLocalLandChargeService")
    def test_searches_to_repeat_top_get(self, mock_sllc, mock_show_paid_repeat, mock_show_free_repeat):
        mock_show_paid_repeat.return_value = True
        mock_show_free_repeat.return_value = True
        ...
        result = self.client.get(url_for("my_account.searches_to_repeat_top"))
        self.assertEqual(result.status_code, 200)
        self.assertIn("Searches we recommend you repeat", result.text)
        mock_sllc.return_value.get_free_search_items_to_repeat.assert_called_with("mock_user", 15, 1)
        mock_sllc.return_value.get_paid_search_items_to_repeat.assert_called_with("mock_user", 15, 1)

    @patch("server.app.has_free_searches_to_repeat")
    @patch("server.app.has_paid_searches_to_repeat")
    @patch("server.views.my_account.SearchLocalLandChargeService")
    @patch("server.views.my_account.calculate_pagination_info")
    def test_paid_searches_to_repeat_get(self, mock_paginate, mock_sllc, mock_show_paid_repeat, mock_show_free_repeat):
        mock_show_paid_repeat.return_value = True
        mock_show_free_repeat.return_value = False
        mock_sllc.return_value.get_paid_search_items_to_repeat.return_value = {
            "pages": 1,
        ...
        mock_paginate.return_value = [], {}, 0
        result = self.client.get(url_for("my_account.paid_searches_to_repeat"))
        self.assertEqual(result.status_code, 200)
        self.assertIn("Official searches", result.text)
        mock_sllc.return_value.get_paid_search_items_to_repeat.assert_called_with("mock_user", 15, 1)
    ...
    @patch("server.views.my_account.SearchLocalLandChargeService")
    @patch("server.views.my_account.calculate_pagination_info")
    def test_free_searches_to_repeat_get(self, mock_paginate, mock_sllc, mock_show_paid_repeat, mock_show_free_repeat):
        mock_show_paid_repeat.return_value = False
        mock_show_free_repeat.return_value = True
        mock_sllc.return_value.get_free_search_items_to_repeat.return_value = {
            "pages": 1,
        ...
        mock_paginate.return_value = [], {}, 0
        result = self.client.get(url_for("my_account.free_searches_to_repeat"))
        self.assertEqual(result.status_code, 200)
        self.assertIn("Free searches", result.text)
        mock_sllc.return_value.get_free_search_items_to_repeat.assert_called_with("mock_user", 15, 1)
        mock_paginate.assert_called_with(
    ...
    def test_free_search_review_found(self, mock_area, mock_audit, mock_sllc):
        mock_sllc.return_value.get_free_search_for_user_by_search_id.return_value = [
            {"search-extent": {"an": "extent"}, "address": "anaddress"}
        ]
