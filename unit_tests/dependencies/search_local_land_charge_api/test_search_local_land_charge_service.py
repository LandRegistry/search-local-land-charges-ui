...
class TestSearchLocalLandChargeApiService(TestCase):
    ...
    @patch("server.dependencies.search_local_land_charge_api." "search_local_land_charge_service.datetime")
    ...
    @patch(
        "server.dependencies.search_local_land_charge_api." "search_local_land_charge_service.LLC1DocumentAPIService"
    )
    def test_check_for_completed_doc_url_ok(self, mock_llc1_api):
        with super_test_context(app):
            ...
            self.assertEqual(
                search_llc_service.check_for_completed_doc_url(mock_search).document_url,
                "someurl",
            )
    ...
    @patch("server.dependencies.search_local_land_charge_api." "search_local_land_charge_service.PaidSearchItem")
    def test_get_paid_search_items_ok(self, mock_paid_item):
        with super_test_context(app):
            ...
            mock_paid_item.from_json_list.return_value = [
                mock_search_item,
                mock_search_item2,
                mock_search_item3,
            ]
    ...
    def test_get_paid_search_items_to_repeat_200(self):
        with super_test_context(app):
            response_json = {"response": "this is json"}
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            g.requests.get.return_value = mock_resp
            g.requests.get.return_value.json.return_value = response_json
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.get_paid_search_items_to_repeat("userid", 20, 1),
                response_json,
            )

    def test_get_paid_search_items_to_repeat_404(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 404
            ...
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(search_llc_service.get_paid_search_items_to_repeat("userid", 20, 1), {})

    def test_get_paid_search_items_to_repeat_500(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 500
    ...
    def test_remove_paid_search_repeat_request_200(self):
        with super_test_context(app):
            response_json = {"response": "this is json"}
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            g.requests.delete.return_value = mock_resp
            g.requests.delete.return_value.json.return_value = response_json
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.remove_paid_search_repeat_request("searchid"),
                response_json,
            )

    def test_remove_paid_search_repeat_request_404(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 404
            ...
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(search_llc_service.remove_paid_search_repeat_request("searchid"), {})

    def test_remove_paid_search_repeat_request_500(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 500
            ...
            with self.assertRaises(ApplicationError):
                search_llc_service = SearchLocalLandChargeService(config)
                search_llc_service.remove_paid_search_repeat_request("anid")
    ...
    def test_get_free_search_items_404(self):
        with super_test_context(app):
            ...
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(search_llc_service.get_free_search_items_to_repeat("userid", 20, 1), {})

    def test_get_free_search_items_to_repeat_200(self):
        with super_test_context(app):
            response_json = {"response": "this is json"}
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            ...
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.get_free_search_items_to_repeat("userid", 20, 1),
                response_json,
            )

    def test_get_free_search_items_to_repeat_404(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 404
    ...
    def test_get_free_search_items_to_repeat_500(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 500
    ...
    def test_get_free_search_item_200(self):
        with super_test_context(app):
            ...
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.get_free_search_for_user_by_search_id("userid", "searchid"),
                [respone_json],
            )
    ...
    def test_get_free_search_for_user_to_repeat_by_search_id_200(self):
        with super_test_context(app):
            response_json = {"response": "this is json"}
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            ...
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.get_free_search_for_user_to_repeat_by_search_id("userid", "searchid"),
                [response_json],
            )

    def test_get_free_search_for_user_to_repeat_by_search_id_404(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 404
    ...
    def test_get_free_search_for_user_to_repeat_by_search_id_500(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 500
    ...
    def test_remove_free_search_repeat_request_200(self):
        with super_test_context(app):
            response_json = {"response": "this is json"}
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            ...
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(
                search_llc_service.remove_free_search_repeat_request("searchid"),
                response_json,
            )

    def test_remove_free_search_repeat_request_404(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 404
            ...
            config = {"SEARCH_LOCAL_LAND_CHARGE_API_URL": "a URL"}
            search_llc_service = SearchLocalLandChargeService(config)
            self.assertEqual(search_llc_service.remove_free_search_repeat_request("searchid"), {})

    def test_remove_free_search_repeat_request_500(self):
        with super_test_context(app):
            g.trace_id = "Help me"
            mock_resp = MagicMock()
            mock_resp.status_code = 500
            ...
            with self.assertRaises(ApplicationError):
                search_llc_service = SearchLocalLandChargeService(config)
                search_llc_service.remove_free_search_repeat_request("anid")
            g.requests.delete.assert_called()
