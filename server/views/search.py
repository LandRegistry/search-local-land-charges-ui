...
def address_search_results():
    ...
    display_addresses, pagination_info, start_index = calculate_pagination_info(
        addresses, "search.address_search_results", ADDRESSES_PER_PAGE, display_page
    )
...
def property_address_selected(address_selected):
    ...
    if address_selected.get("uprn"):
        ...
        if features:
            all_features = {"type": "FeatureCollection", "features": features}
...
def search_by_coordinates():
    ...
    if form.validate_on_submit():
        ...
        session["zoom_to_location"] = [
            float(form.eastings.data),
            float(form.northings.data),
        ]
...
def search_by_title_number():
    ...
    if form.validate_on_submit():
        ...
        if response.status_code == 200:
            ...
            raise ApplicationError(
                f"Failed to retrieve search information for title {form.title_number.data.strip()}",
                "FTITLE01",
                500,
            )
    ...
    if form.validate_on_submit():
        ...
        if response_inspire_search.get("status") == 200:
            ...
            current_app.logger.error(
                "Failed to search by area, status code {}".format(response_inspire_search.get("status"))
            )
...
def inspire_search_results():
    ...
    return render_template(
        "results-from-inspire-search.html",
        back_link=True,
        charge=charge_obj,
...
def confirm_search_area():
    ...
    if form.validate_on_submit():
        if authority_data["flag"] == "pass":
            ...
            return search_by_area(
                session["search_state"].search_extent,
                session["search_state"].address,
                warning=True,
            )
        else:
...
def search_by_area(extent, address, warning=False):
    ...
    if session["search_state"].charges:
        ...
        charge_ids = []
    free_search = SearchLocalLandChargeService(current_app.config).save_free_search(
        user_id, charge_ids, extent, address
    )
