    ...
    if "profile" in session and "user_id" in session["profile"]:
        ...
        if from_homepage:
            ...
            with force_locale(g.locale):
                response = make_response(render_template("sign-in.html", form=form, redirect_uri=redirect_uri))
                return set_language_cookie(response, g.locale)
        else:
            return render_template("sign-in.html", form=form, redirect_uri=redirect_uri)


def check_user(form):
    success, result = AuthenticationApi().authenticate(
        form.username_password.password.data, form.username_password.username.data
    )
    if success:
        userinfo = validate(
            f"{config.AUTHENTICATION_URL}/v2.0/authentication/validate",
            result,
            g.requests,
        )
    ...
    if result.get("locked", False):
        message = _(
            "You tried to log in using an invalid username or password. For security reasons, "
        ...
        AuditAPIService.audit_event("User authentication failed x10 - account locked", supporting_info=result)
    else:
