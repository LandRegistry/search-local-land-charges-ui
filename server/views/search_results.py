...
def results():
    ...
    if form.validate_on_submit():
        ...
        return redirect(
            url_for(
                ...
                enc_search_id=form.enc_search_id.data,
            )
        )
    ...
    display_charges, pagination_info, start_index = calculate_pagination_info(
        charges, "search_results.results", CHARGES_PER_PAGE, current_page
    )
...
def sort_charges(charges):
    ...
    sorted_charges = [
        {
            "group-type": (charge.charge_type if charge.charge_type in order else "Other"),
            "charge": charge,
        }
...
def format_land_comp(charge, content):
    if (
        charge.land_compensation_paid
        or charge.land_compensation_amount_type
        or charge.land_capacity_description
        or charge.amount_of_compensation
    ):
...
def format_lon(charge, heading, content):
    ...
    if charge.charge_geographic_description:
        content[f"{_('Location')}\n{_('(dominant building)')}"] = charge.charge_geographic_description.splitlines()
    else:
    ...
    if "temporary-certificate" in charge.documents_filed:
        content[f"{_('Start date')}\n{_('(temporary certificate)')}"] = charge.format_date_for_display(
    ...
    if "definitive-certificate" in charge.documents_filed:
        content[f"{_('Start date')}\n{_('(definitive certificate)')}"] = charge.format_date_for_display(
            "tribunal_definitive_certificate_date"
        )
        content[f"{_('Expiry date')}\n{_('(definitive certificate)')}"] = charge.format_date_for_display(
            "expiry_date", default=_("Does not expire")
        )
    content[_("Law")] = charge.format_field_for_display("statutory_provision")
    content[_("Legal document")] = charge.format_field_for_display("instrument")
    content[_("HM Land Registry reference")] = charge.format_llc_ref_for_display("local_land_charge")
