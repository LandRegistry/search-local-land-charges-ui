...
paid_search = Blueprint(
    "paid_search",
    __name__,
    url_prefix="/paid-search",
    template_folder="../templates/paid-search",
)
    ...
    if form.validate_on_submit():
        if form.find.data:
            ...
            if len(address_list) == 0:
                form.search_postcode.errors.append(_("No results found"))
        else:
        ...
        if form.address_matches.data == "yes":
            ...
            if form.search_area_description.data is None or form.search_area_description.data.strip() == "":
                ...
                return continue_to_payment(
                    form.search_area_description.data.strip(),
                    search_state,
                    enc_search_id,
                )
...
def search_description_find(form):
    ...
    if form.search_postcode.data is None or form.search_postcode.data.strip() == "":
        ...
        results = SearchByPostcode(current_app.logger).process(form.search_postcode.data.strip(), current_app.config)
...
def continue_to_payment(search_area_description, search_state, enc_search_id):
    ...
    payment_request = gov_pay_service.request_payment(
        format_fee_for_gov_pay(config.SEARCH_FEE_IN_PENCE),
        reference_num,
...
def process_pay(enc_search_id):
    ...
    if payment_state.state["status"] == "success":
        ...
        response = search_by_area_processor.process(
            search_session["search_state"].search_extent, results_filter="cancelled"
        )
...
def get_paid_search(enc_search_id):
    ...
    if (
        ...
        and not refresh
        and (
            search_session["payment_state"].payment_id is None
    ...
    if search_session["search_state"].search_reference is None:
        ...
        search_ref = PaidSearchUtils.request_search_generation(
        ...
        return render_template(
            ...
            payment_state=search_session["payment_state"],
            refresh=refresh,
            enc_search_id=enc_search_id,
        )
    ...
    if not paid_search:
        try:
            paid_search = PaidSearchUtils.get_search_result(
                search_session["search_state"].search_reference,
                search_session["payment_state"].payment_id,
                search_session["search_state"].search_extent,
                search_session["search_state"].address,
                search_session["search_state"].charges,
    ...
    if not paid_search:
        return render_template(
            "paid-search-query.html",
            payment_state=search_session["payment_state"],
            refresh=refresh,
            reload_time=config.PDF_GENERATION_POLL,
            enc_search_id=enc_search_id,
        )
...
def download_charges(enc_search_id):
    ...
    if "json" == download_format:
        ...
        return send_file(
            ...
            download_name="LocalLandCharges_{}.json".format(date),
            as_attachment=True,
        )
        ...
        return send_file(
            csvfile,
            mimetype="text/csv",
            download_name="LocalLandCharges_{}.csv".format(date),
            as_attachment=True,
        )
        ...
        return send_file(
            ...
            download_name="LocalLandCharges_{}.xml".format(date),
            as_attachment=True,
        )
    else:
...
@authenticated
def supporting_document_list(enc_search_id, redirect_url=None):
    ...
    charges = [
        calc_display_id(charge["local-land-charge"])
        for charge in search_session["paid_search_item"].charges
        if "documents-filed" in charge and len(charge["documents-filed"].keys()) > 0
    ]
    charges.sort()

    display_charges, pagination_info, start_index = calculate_pagination_info(
        charges,
        "paid_search.supporting_document_list",
...
def charge_supporting_documents(enc_search_id, charge_id):
    ...
    documents_url = storage_api_service.get_external_url(one_document["subdirectory"], one_document["bucket"])

    if not request.is_xhr:
        flash(
            _(
                "Downloading light obstruction notice documents for %(charge_id)s. "
                'If download does not start <a class="govuk-link" href="%(documents_url)s">click here.</a>',
                charge_id=charge_id,
                documents_url=documents_url,
            )
        )

        return supporting_document_list(enc_search_id, documents_url)
    return Response(documents_url, mimetype="text/plain", status=200)


@paid_search.route("/<enc_search_id>/supporting_documents/<charge_id>/error")
@authenticated
def charge_supporting_documents_error(enc_search_id, charge_id):
    raise ApplicationError(
        f"Failed to create zip of supporting documents for {charge_id}",
        "PAYSZIP01",
        500,
    )
