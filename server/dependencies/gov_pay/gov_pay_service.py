...
class GovPayService(object):
    ...
    def request_payment(self, amount, reference, description, enc_search_id):
        ...
        if payment_request.status_code != 201:
            raise ApplicationError(f"Fail response from gov pay API: {repr(payment_request.json())}", "GOVP01",
    ...
    def get_payment(self, payment_id):
        ...
        if payment_info.status_code != 200:
            raise ApplicationError(f"Fail response from gov pay API: {repr(payment_info.json())}", "GOVP01",
