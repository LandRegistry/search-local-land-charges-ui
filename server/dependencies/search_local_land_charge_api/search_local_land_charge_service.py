...
class SearchLocalLandChargeService(object):
    ...
    def save_users_paid_search(self, user_id, paid_search_item):
        ...
        response = g.requests.post(
            "{}/users/{}/paid-searches".format(self.url, user_id),
    ...
    def get_paid_search_items(self, user_id):
        ...
        if response.status_code == 404:
            ...
            raise ApplicationError(
                "Error calling search-local-land-charge-api",
                "SLLC02",
                500,
            )
    ...
    def get_paid_search_items_to_repeat(self, user_id, page_size, page):
        url = f"{self.url}/users/{user_id}/paid-searches/request_repeated"
    ...
    def remove_paid_search_repeat_request(self, search_id):
        url = f"{self.url}/users/paid-searches/{search_id}/request_repeat"
        current_app.logger.info(f"Calling Search Local Land Charge API at {url}.")
        response = g.requests.delete(url)
    ...
    def get_service_messages(self):
        ...
        for message_obj in response.json()["messages"]:
            if g.locale == "cy":
                ...
                hyperlink_message = (
                    message_obj.get("hyperlink-message-cy")
                    or message_obj.get("hyperlink-link-cy")
                    or message_obj.get("hyperlink-link-en")
                )
                hyperlink_link = message_obj.get("hyperlink-link-cy") or message_obj.get("hyperlink-link-en")
            else:
                message = message_obj.get("message-en")
                hyperlink_message = message_obj.get("hyperlink-message-en") or message_obj.get("hyperlink-link-en")
    ...
    def get_free_search_items_to_repeat(self, user_id, page_size, page):
        url = f"{self.url}/free-searches/user_id/{user_id}/request_repeated"
        current_app.logger.info(f"Calling Search Local Land Charge API at {url}")
        response = g.requests.get(url, params={"page": page, "per_page": page_size})
    ...
    def get_free_search_for_user_to_repeat_by_search_id(self, user_id, search_id):
        url = f"{self.url}/free-searches/user_id/{user_id}/request_repeated/{search_id}"
        current_app.logger.info(f"Calling Search Local Land Charge API at {url}.")
        response = g.requests.get(url)
    ...
    def remove_free_search_repeat_request(self, search_id):
        url = f"{self.url}/free-searches/{search_id}/request_repeat"
        current_app.logger.info(f"Calling Search Local Land Charge API at {url}.")
        response = g.requests.delete(url)
