...
class LocalAuthorityAPIService(object):
    ...
    def get_organisations(params=None):
        ...
        response = g.requests.get('{}/v1.0/organisations'.format(LOCAL_AUTHORITY_API_URL),
                                  params=params)

        if response.status_code != 200:
            current_app.logger.exception(
                'Failed to get organisations from local-authority-api. Message: {}'
    ...
    def plus_minus_buffer(extent):
        ...
        response = g.requests.post('{}/v1.0/local-authorities/plus_minus_buffer'
                                   .format(LOCAL_AUTHORITY_API_URL),
        ...
        if response.status_code == 200:
            ...
            raise ApplicationError("Failed to determine if the given extent is within a migrated area", "LAUTH04", 400)
