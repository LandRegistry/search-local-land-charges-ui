...
dicttoxml2.LOG.setLevel(logging.ERROR)
...
class PaidResultsFormatter(object):
    ...
    light_obstruction_notice_fieldnames = [
        "applicants",
    ...
    def land_compensation_charge_fields(self, charge):
        if (
            charge.get("land-compensation-paid")
            or charge.get("land-compensation-amount-type")
    ...
    def light_obstruction_notice_fields(self, charge):
        ...
        if "temporary-certificate" in fields["documents-filed"]:
            fields["tribunal-temporary-certificate-date"] = self.format_field(
                charge.get("tribunal-temporary-certificate-date")
            )
            fields["tribunal-temporary-certificate-expiry-date"] = self.format_field(
                charge.get("tribunal-temporary-certificate-expiry-date")
            )

        if "definitive-certificate" in fields["documents-filed"]:
            fields["expiry-date"] = self.format_field(charge.get("expiry-date", ""))
            fields["tribunal-definitive-certificate-date"] = self.format_field(
                charge.get("tribunal-definitive-certificate-date")
            )
