...
def calculate_pagination_info(
    ...
    end_index = no_of_items - 1 if current_page == no_of_pages else (items_per_page * current_page) - 1
    ...
    if current_page > 1:
        ...
        if current_page > 2:
            pagination_info["items"].append({"number": 1, "href": url_for(paginate_url, **paginate_args)})
        if current_page > 3:
            pagination_info["items"].append({"ellipsis": True})
