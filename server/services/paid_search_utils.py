...
class PaidSearchUtils(object):
    ...
    def request_search_generation(search_extent, address, parent_search_id=None, contact_id=None):
        ...
        llc1_response = document_service.generate(search_extent, address, parent_search_id=parent_search_id,
                                                  contact_id=contact_id)
        return llc1_response.get("search_reference", None)

    @staticmethod
    def pre_associate_search(search_ref, payment_id, search_extent, address, charges, payment_provider,
                             card_brand, amount, reference, parent_search_id=None):

        user_id = session['profile']['user_id']

        paid_search = PaidSearchUtils.build_search_item(int(search_ref.replace(" ", "")), None, payment_id,
                                                        search_extent, address, charges,
                                                        payment_provider, card_brand, user_id, amount, reference,
                                                        parent_search_id)
        ...
        ReportAPIService.send_event("llc1_search", {
    ...
    @staticmethod
    def get_search_result(search_ref, payment_id, search_extent, address, charges, payment_provider,
                          card_brand, amount, reference, parent_search_id=None):
        document_service = LLC1DocumentAPIService(current_app.config)
        ...
        paid_search = PaidSearchUtils.build_search_item(int(llc1_response['reference_number'].replace(" ", "")),
                                                        llc1_response['document_url'],
                                                        payment_id, search_extent, address, charges,
                                                        payment_provider, card_brand, user_id, amount, reference,
                                                        parent_search_id)
    ...
    @staticmethod
    def build_search_item(search_id, document_url, payment_id, search_extent, address, charges,
                          payment_provider, card_brand, user_id, amount, reference, parent_search_id=None):
