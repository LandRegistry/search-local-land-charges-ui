...
${BOLD}1.${NORMAL} Trying to install a new package:
  ...
  * Inside the virtual machine, run ${BOLD}rebuild search-local-land-charge-ui${NORMAL}
...
${BOLD}2.${NORMAL} Trying to npm install *everything*, because "that's what you normally do"
  ...
  * Be aware that these will take precedence over those housed in the
...
${BOLD}3.${NORMAL} Trying to ${BOLD}npm link${NORMAL} a module you are developing:
  * You can't! Because Docker volumes do not support symlinks.
    ...
    Things inside node_modules will take precedence over those installed inside
    the container.


${BOLD}Now you have read all this, do you still want to continue?${NORMAL}
END
)
